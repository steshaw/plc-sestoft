...
let r2 = y > 0.0 && 1.0/y > 7.0

let another_not = not
let r3 = another_not false

let r4 = if 3 < 4 then 117 else 118
...
let r7 = makejunk "Vice Chancellor" "Tofte"
...
exception IllegalHourException
let mins1 h =
    if h < 0 || h > 23 then raise IllegalHourException
    else h * 60
let r14 = mins1 23

let r15 = try Some(mins1 25) with IllegalHourException -> None
...
let r16 = mins2 23
let r16' = try Some(mins2 25) with _ -> None
...
let r17 = mins3 23
let r17' = try Some(mins3 25) with _ -> None
...
let r18 = try Some(getphone1 (Student "Niels")) with _ -> None
...
let r18' = getphone2 (Student "Niels")
...
let rec sumtree0 t =
    match t with
    | Lf -> 0
    | Br(v, t1, t2) -> v + sumtree0 t1 + sumtree0 t2

let t1sum = sumtree0 t1'
...
let r20 = len' ["Oslo"; "Aarhus"; "Gothenburg"; "Copenhagen"]
...
let r22 = bind1 [("age", 47)] ("phone", 5083)
...
let r23 = map0 mul2 [4.0; 5.0; 89.0]

let r24 = map0 isLarge [4.0; 5.0; 89.0]
...
let r26 = map0 (fun x -> 2.0 * x) [4.0; 5.0; 89.0]

let r27 = map0 (fun x -> 10.0 < x) [4.0; 5.0; 89.0]
...
let rec filter0 p xs =
    match xs with
    | []    -> []
    | x::xr -> if p x then x :: filter0 p xr else filter0 p xr

let onlyEven = filter0 (fun i -> i%2 = 0) [4; 6; 5; 2; 54; 89]
...
let sum xs   = foldr (+) xs 0

let prod xs  = foldr (*) xs 1

let map g xs = foldr (fun x res -> g x :: res) xs []

let listconcat xss = foldr (@) xss []

let stringconcat xss = foldr (+) xss ""
